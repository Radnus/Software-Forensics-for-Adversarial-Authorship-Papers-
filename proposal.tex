...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
	\State \textbf{Input}{ F: original feature set}
	\State \textbf{Input}{ N: size of feature collection}
	\State \textbf{Input}{ D: dimension of feature}
	\State \textbf{Output}{ S: optimal feature subset}
	\State \textbf{Initialize}{ each individual data point in the feature collection}
	\State \textbf{Calculate}{ the matrix R of correlation coefficients between features in F}
	\State \textbf{Calculate}{ the contribution of each feature in F by R}
	    \While{The termination condition of the iteration is not satisfied }
		\For{$i=1 \textbf{ to } N$}
			\State \textbf{Calculate}{ the fitness value of the individual data point using classifiers}
			\State \textbf{Update}{ the historical best position of the individual data point}
			\EndFor
		\State \textbf{Update}{ the historical best position of the individual data point}
		\For{$i=1 \textbf{ to } N$}
		    \For{$i=1 \textbf{ to } D$}
                \State \textbf{Update}{ the velocity of the individual data point}
                \State \textbf{Update}{ the position of the individual data point combining the w value of each feature}
		        \EndFor
		    \EndFor
	    \EndWhile
	\State \textbf{Output}{ the optimal position (optimal feature subset)}
	\end{algorithmic} 
\end{algorithm}

The pseudocode is shown in Algorithm 1, which starts with the initialization of each individual data point in the feature collection.  Then the correlation coefficients between features and the contribution of each feature in all feature sets are calculated to obtain the optimal initial value of the individual and the feature collection, as defined by lines 6 and 7.  The search process will stop and output the optimal position of the feature collection, if the process comes to a stopping condition; otherwise, it will execute circularly.  Two stopping conditions are defined—either when the error reaches the preset requirement or when the number of iterations exceeds the maximum allowable number of iterations.  Lines 8–20 define the detailed cyclic process to get the optimal feature subset, which contains updating the individual data points’ velocity and position, calculating the fitness value of the individual data point using classifiers, and updating the historical best position of the individual data point and the optimal position of the feature collection.  The optimal feature subset will be selected after finishing this cyclic process.
